    ...
    @include('admin.lightnavbar')
      <div class="container-fluid">
        ...
        @endif
          ...
          <div class="card w-100">
              <div class="card-body p-4">
                <h5 class="card-title fw-semibold mb-4">Gelişimsel Değerlendirme Formu Görüntüle</h5>
                <form action="{{url('lightgdfgoruntule')}}" method="POST" >
                @csrf
                <div class="mb-3">
                      <label for="exampleInputEmail1" class="form-label">Çocuk Adı :</label>
                     <select class="form-select" name="cocukadi" >
                        <option >--Seçiniz--</option>
                        @foreach ($gelisimsel as $gdf)
                        <option value="{{$gdf->cocukadi}}">{{$gdf->cocukadi}}</option>
                ...
                <input type="submit" style="color:white;background-color:#5D87FF;" value="Görüntüle"  class="btn btn-outline-primary">
                </div>
            </form>
              </div>
            </div>
